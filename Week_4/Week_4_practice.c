...
void inputGoodsName(char name[][20]);
void statusGoods(int* stock, int* sale, char name[][20]);

int main() {
	int stock[GOODS] = { 0 };
	int sale[GOODS] = { 0 };
	char name[GOODS][20] = { 0 };
	int num;
...
void statusGoods(int* stock, int* sale, char name[][20]) {
	...
	printf("가장 많이 판매된 상품: ID %d, 상품명: %s, 판매량 %d\n", maxSale + 1, name[maxSale], sale[maxSale]);
	printf("가장 적게 판매된 상품: ID %d, 상품명: %s, 판매량 %d\n", minSale + 1, name[minSale], sale[minSale]);

	for (int i = 0; i < GOODS; i++) {
		if (stock[i] - sale[i] <= 2) {
			printf("상품 ID %d: 상품명 : %s, 재고부족(%d)\n", i + 1, name[i], stock[i] - sale[i]);
		}
	}
}
